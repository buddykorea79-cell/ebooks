...
- **도서 편집**(`/book/:id/edit`): 탭 구조
  - 기본 정보: 제목/분류/유형/설명/**본문 형식(HTML·마크다운)**/공개, 표지 이미지 업로드(Storage `covers` 버킷)
  - 메뉴 관리: 트리 편집(추가/이름변경/삭제/이동/들여쓰기) + CodeMirror 편집·미리보기
...
- **뷰어**(`/book/:id`): 사이드바 목차(모바일은 드로어) + iframe 샌드박스 렌더링(HtmlViewer).
  마크다운 도서는 `marked`로 HTML 변환 + 기본 문서 스타일을 입혀 렌더링
...
- **검색**(`/search`): 헤더 검색창(모바일은 🔍 아이콘)으로 어느 페이지에서나 진입.
  ...
  태그 제거 후 검색어 주변 스니펫을 하이라이트해 해당 메뉴로 바로 이동
...
books                 도서 (owner_id, category_id, type→book_types FK, title,
                      ...
                      content_format('html'|'markdown'), is_published,
