/**
 * POST /api/ai — BizRouter(https://bizrouter.ai) Chat Completions 연동.
 *
 * 브라우저에서 직접 호출하면 API 키가 번들에 노출되므로 호출은 반드시 서버에서만 한다.
 * 운영에서는 이 파일이 그대로 Vercel 서버리스 함수(맨 아래 default export)가 되고,
 * 로컬 개발에서는 vite.config.ts의 dev 미들웨어가 아래 runAi를 직접 호출한다.
 *
 * 위쪽 절반(runAi 등)은 Node/Vercel 객체를 다루지 않고 순수 값만 주고받으므로
 * 두 경로에서 동작이 갈리지 않는다.
 *
...
// ---------------------------------------------------------------
// Vercel 서버리스 함수 (POST /api/ai)
//
// 이 파일은 상대 경로 import를 두지 않는다. package.json이 "type": "module"이라
// Vercel에서 ESM으로 실행되는데, Node ESM은 확장자 없는 상대 import를 해석하지
// 못해 모듈 로드 단계에서 FUNCTION_INVOCATION_FAILED로 죽기 때문이다.
// 로컬 dev 미들웨어(vite.config.ts)는 위의 named export를 가져다 쓴다.
// ---------------------------------------------------------------

/** node:http 타입을 import하지 않고 필요한 모양만 구조적으로 정의한다 */
